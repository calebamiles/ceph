...
static bool get_auth_header(struct req_state *s, string& dest, bool qsr)
{
  ...
  std::string admin_sub_resource = s->args.get_admin_sub_resource();
  size_t resource_len = admin_sub_resource.size();

/*
  if (resource_len > 0) {
    size_t pos = canon_resource.find(admin_sub_resource) -1;
    size_t len = admin_sub_resource.size() + 1;
